    ...
    for (int di = -1; di <= 1; di++)
    {
        for (int dj = -1; dj <= 1; dj++)
        {
            ...
            if (i + di >= 0 && i + di < height && j + dj >= 0 && j + dj < width)
            {
                ...
                greenX += multiplierX * image[i + di][j + dj].rgbtGreen;
                blueX += multiplierX * image[i + di][j + dj].rgbtBlue;

                int multiplierY = Gy[di + 1][dj + 1];
                redY += multiplierY * image[i + di][j + dj].rgbtRed;
                greenY += multiplierY * image[i + di][j + dj].rgbtGreen;
                blueY += multiplierY * image[i + di][j + dj].rgbtBlue;
            }
        }
    }

    RGBTRIPLE returnPixel;
    returnPixel.rgbtRed = max(round(sqrt(redX * redX + redY * redY)));
    returnPixel.rgbtGreen = max(round(sqrt(greenX * greenX + greenY * greenY)));
    returnPixel.rgbtBlue = max(round(sqrt(blueX * blueX + blueY * blueY)));
    return returnPixel;
}
// Detect edges
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int row = 0; row < height; row++)
    {
        for (int column = 0; column < width; column++)
        {
            //Running the function of "get_edged_pixel"
            tempImage[row][column] = get_edged_pixel(row, column, height, width, image);
        }
    }
